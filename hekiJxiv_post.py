...
def main(switches, logfiles, pt_mode, max_post=None, num_last_days=None):
...
def update_log(logfiles, cat, total, preprint_id, result, pt_mode):
    ...
    if not filename:
        return None
    # log files are never created.
    if not os.path.isfile(filename):
        print("**log file does not exist, not logged: " + filename
              + (", preprint id: " + preprint_id if preprint_id else ""))
        return None
    # an empty file gets the header first.
    header = os.path.getsize(filename) == 0
    df.to_csv(filename, mode="a", header=header, index=None)
...
def newentries(
    ...
    if pt_mode:
        ...
        if entry is None or any(
            ...
            return None
        # a missing log file stops this label.
        missing = [entry[key] for key in required if key != "username"
                   and not os.path.isfile(entry[key])]
        if missing:
            print("**nothing posted for " + cat + ": log file(s) missing: "
                  + ", ".join(missing)
                  + ". Create them first (an empty file will do).")
            return None
    ...
    try:
        entries = hJd.new_records(cat, announced, max_post, num_last_days)
    except Exception:
        print("\n**error for retrieval**\nlabel: " + cat)
        traceback.print_exc()
